...
export function buildReceivables(store,commissionData,excellentSnapshot=null,periodStart='2026-07-01',selectedAgent='Francesco'){
...
export function receivablesHtml(data){
  ...
  <div class="card commission-receivables-card">
    ...
    ${byCategory.map(([cat,items])=>`<details class="receivable-group">
      ...
      ${items.sort((a,b)=>String(a.paymentMonth).localeCompare(String(b.paymentMonth))).map(r=>`<div class="receivable-line${r.category==='Community'?' receivable-link':''}" ${r.category==='Community'?'data-open-community-prizes':''}><div><strong>${r.client}</strong><small>Pagamento ${r.paymentMonth}${r.detail?' · '+r.detail:''}</small></div><b>${money(r.estimated)}</b></div>`).join('')}
    </details>`).join('')}
  </div>`;
}
