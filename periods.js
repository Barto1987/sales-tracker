...
export function ensurePeriodState(store,k){
  store.periodStates=store.periodStates||{};
  if(!store.periodStates[k]){
    // Stato automatico: non deve mai prevalere su uno stato Cloud già verificato/chiuso.
    store.periodStates[k]={status:'working',manual:false,createdAt:new Date().toISOString(),updatedAt:''};
  }
  if(store.periodStates[k].manual==null)store.periodStates[k].manual=store.periodStates[k].status!=='working';
  return store.periodStates[k];
}
