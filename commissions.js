import {matchEasyRent} from './easy-rent-listino.js?v=31517';
import {recognizeM2MProduct} from './m2m-listino.js?v=31517';
import {COMMISSION_RULE_SETS,commissionRuleSetForDate,activeCommissionRuleSet} from './commission-rules.js?v=31517';
// SmartTracker 3.15.17 — prima base del motore Provvigioni.
...
function quarterAgencyPaymentMonth(date){
  ...
  let y=d.getFullYear(),m=qEndMonth+2;
  if(m>12){y+=1;m-=12}
...
function quarterKeyFromDate(date){const [y,m]=String(date||'').slice(0,7).split('-').map(Number);return y&&m?`${y}-Q${Math.ceil(m/3)}`:''}
function agencyResult(store,date,key){const q=quarterKeyFromDate(date);return store.settings?.agencyQuarterResults?.[q]?.[key]||'pending'}
...
function ruleFor(s,date='2026-07-01'){
  ...
  if(isDigital(s)) return {type:'Digital',manual:true,ruleSetId:rs?.id,reason:'Prodotto Digital riconosciuto, ma manca la specifica regola provvigionale base.'};
...
export function commissionsForPeriod(store,start,end,agent='Francesco'){
  ...
  for(const c of (store.contracts||[]).filter(c=>active(c)&&agentOf(c)===agent&&inRange(c.date,start,end))){
    for(const s of c.services||[]){
      ...
      const agencyExtra=(agencyState==='yes'&&(agencyGroup==='digital'||access.unlocked))?inflow*Number(rule.agencyCanons||0):0;
